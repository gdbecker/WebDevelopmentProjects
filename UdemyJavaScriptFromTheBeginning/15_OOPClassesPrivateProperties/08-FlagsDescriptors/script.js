// [[Configurable]] - if `true`, the property can be deleted and these attributes can be modified, otherwise not
// [[Enumerable]] - if `true`, the property will be returned in a `for...in` loop, otherwise not
// [[Writable]] - if `true`, the value of the property can be changed, otherwise not
// [[Value]] - the value of the property

let d = Object.getOwnPropertyDescriptor(Math, 'PI');
console.log(d); // show the flag values

const rectObj = {
	name: 'Rectangle 1',
	width: 10,
	height: 10
}

d = Object.getOwnPropertyDescriptor(rectObj, 'name');
console.log(d); // show the flag values

Object.defineProperty(rectObj, 'name', {
	writable: false,
	configurable: false,
	enumerable: false
});

d = Object.getOwnPropertyDescriptor(rectObj, 'name');
console.log(d); // show the flag values

// Won't work because we changed the flags for 'name'
rectObj.name = 'New Name';
console.log(rectObj);

// Won't show the 'name' because we made 'name' enumerable = false
for (let [key, value] of Object.entries(rectObj)) {
	console.log(`key: ${key} : value: ${value}`);
}

// get flags for all object properties
console.log(Object.getOwnPropertyDescriptors(rectObj));
